...
def simulate_control():
    ...
    for it in range(sim_iter):
        ...
        k2 = calculate_time_varying_k(desired_state, robot_state2, v0=5, beta=1)
        current_input2 = k2 * (desired_state - robot_state2)

        k3 = calculate_time_varying_k(desired_state, robot_state3, v0=5, beta=3)
...
if __name__ == '__main__':
    ...
    ax.plot(t, state_history2[:,0], label='px, v0=5, beta=1 [m]')
    ax.plot(t, state_history2[:,1], label='py, v0=5, beta=1 [m]')
    ax.plot(t, state_history2[:,2], label='theta, v0=5, beta=1 [rad]')
    ax.plot(t, state_history3[:,0], label='px, v0=5, beta=3 [m]')
    ax.plot(t, state_history3[:,1], label='py, v0=5, beta=3 [m]')
    ax.plot(t, state_history3[:,2], label='theta, v0=5, beta=3 [rad]')
    ax.plot(t, goal_history[:,0], ':', label='goal px [m]')
    ax.plot(t, goal_history[:,1], ':', label='goal py [m]')
    ax.plot(t, goal_history[:,2], ':', label='goal theta [rad]')
    ax.set(xlabel="t [s]", ylabel="state")
    ...
    fig2 = plt.figure(5)
    ax = plt.gca()
    ax.plot(t, input_history[:,0], label='vx, v0=5, beta=0.5 [m/s]')
    ax.plot(t, input_history[:,1], label='vy, v0=5, beta=0.5 [m/s]')
    ax.plot(t, input_history[:,2], label='omega, v0=5, beta=0.5 [rad/s]')
    ax.plot(t, input_history2[:,0], label='vx, v0=5, beta=1 [m/s]')
    ax.plot(t, input_history2[:,1], label='vy, v0=5, beta=1 [m/s]')
    ax.plot(t, input_history2[:,2], label='omega, v0=5, beta=1 [rad/s]')
    ax.plot(t, input_history3[:,0], label='vx, v0=5, beta=3 [m/s]')
    ax.plot(t, input_history3[:,1], label='vy, v0=5, beta=3 [m/s]')
    ax.plot(t, input_history3[:,2], label='omega, v0=5, beta=3 [rad/s]')
    ax.set(xlabel="t [s]", ylabel="control input")
    plt.legend()
    plt.grid()
