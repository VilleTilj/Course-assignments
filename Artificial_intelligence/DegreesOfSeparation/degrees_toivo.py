...
def shortest_path(source, target):
    ...
    S = {}
    for id in people.items():
        S[id[0]] = Node("unvisited", [], id[0])

    # Initialize the queue with source being the first to be explored
    Q = QueueFrontier()
    S[source].state = "visited"
    S[source].parent = []
    Q.add(S[source])
    ...
    while not Q.empty():
        ...
        if v.action == target:
            ...
            while True:
                if not v.parent:
                    break
                # Reformat the tuples to have the movie that the stars were in, but have the proper id of the node
                to_return.append((v.parent[0], v.action))
                v = S[v.parent[1]]
            ...
            neighbors = neighbors_for_person(v.action)
            for neighbor in neighbors:

                if S[neighbor[1]].state == "unvisited":

                    S[neighbor[1]].state = "visited"
                    # Parent is a tuple of the movie the stars were in, and the parent of this node
                    S[neighbor[1]].parent = (neighbor[0], v.action)
                    Q.add(S[neighbor[1]])
